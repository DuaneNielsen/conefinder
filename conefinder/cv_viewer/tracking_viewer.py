from conefinder.cv_viewer.utils import *
...
class TrackingViewer:
    ...
    def to_cv_point(self, x, z):
        ...
        if isinstance(x, float) and isinstance(z, float):
            ...
            if np.isnan(new_position.get()[0]).any() or np.isnan(new_position.get()[1]).any():
                print('new_position has NaN')
                raise Exception('position sensor returned NaN')
            else:
